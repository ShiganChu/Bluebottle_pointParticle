 ...
 *  hardcoded, but does depend on the pointicular device being used.
 ******
 */
 ...
 *  device.  Must be hardcoded, but does depend on the pointicular device being
 *  used.
 ******
 */
...
/****f* bluebottle/cuda_point_BC_star()
 * NAME
 *  cuda_point_BC_star()
 * USAGE
 */
void cuda_point_BC_star(void);
/*
 * FUNCTION
 *  Enforce boundary conditions in intermediate velocity fields.  *See note
 *  in cuda_BC().
...
/****f* bluebottle/cuda_point_BC_p()
 * NAME
 *  cuda_point_BC_p()
 * USAGE
 */
void cuda_point_BC_p(int dev);
/*
 * FUNCTION
...
/*
 * FUNCTION
 *  Build the point_particle cages on the devices and generate phase array.
 ******
 */

/****f* bluebottle/cuda_point_BC()
 * NAME
 *  cuda_point_BC()
 * USAGE
 */
void cuda_point_BC(void);
/*
 * FUNCTION
 *  Apply the point_particle velocity boundary conditions to the flow domain.
 ******
 */
...
/****f* bluebottle/cuda_move_points_sub()
 * NAME
 *  cuda_move_points_sub()
 * USAGE
 */
void cuda_move_points_sub(void);
/*
 * FUNCTION
 *  Calculate new point_particle velocities and positions for sub-timestep implicit
 ...
 * USAGE
 */
void seeder(int N, real a, real d, int o, int t, int r);
/*
 * FUNCTION
 *  Randomly seed N point_particles in the domain. To use this function, run
 *  'bluebottle -s N a d o t r', using the arguments as defined below. A new
 *  file called point_seeder.config will be created and the main bluebottle
 *  simulation code will not be run. To run a simulation using this input file,
 *  change its name to point.config and run bluebottle normally.
 * ARGUMENTS
 *  * N -- the number of point_particles
 *  * a -- the radius of the point_particles
 *  * d -- the density of the point_particles (rho)
