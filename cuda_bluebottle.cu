...
//TODO require further verification
//omega_x
    for(k = dom[dev].Gfx.ksb; k < dom[dev].Gfx.keb; k++) {
      for(j = dom[dev].Gfx.jsb; j < dom[dev].Gfx.jeb; j++) {
        for(i = dom[dev].Gfx.isb; i < dom[dev].Gfx.ieb; i++) {
...
void Test1()
{
 #pragma omp parallel num_threads(nsubdom)
  {
  real *d_maxarr = NULL;
  checkCudaErrors(cudaMalloc((void**)&d_maxarr, 10*sizeof(real)));
...
{
real H=(Dom.ze-Dom.zs)/2.0;
real omega=.5*(PI/H)*(PI/H);
real C_r=1,C_i=0;
gradP.x=gradP.xm*(C_r*cos(omega*ttime)+C_i*sin(omega*ttime));
//gradP.x=0;
}
else
{
...
void cuda_move_parts_sub()
{
  ...
  #pragma omp parallel num_threads(nsubdom)
  {
    ...
    if(nparts > 0) {
      ...
      // if there are n particles in a close group, repeat this n times 
      real *forces;
...
//for(int l = 0; l < 10; l++) {
      ...
      // repeat to here n times 

      //move_parts_b<<<numBlocks, dimBlocks>>>(_dom[dev], _parts[dev], dt, dt0, g);
