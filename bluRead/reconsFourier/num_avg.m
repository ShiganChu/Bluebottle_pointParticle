...
reali=3;%the number of realizations
...
%  NQ(j)=nq_0+sum(cos_coeff.*cos(K_L*yy(j)))-i*sum(sin_coeff.*sin(K_L*yy(j)));
  NQ_density(j)=COS_0+sum(cesaro_coeff.*COS_COEFF.*cos(K_L*yy(j)))-i*sum(cesaro_coeff.*SIN_COEFF.*sin(K_L*yy(j)));
end
...
handle=figure;
plot(yy,NQ./NQ_density*V/N,'-');
xlabel('x');
ylabel('num density');
title(['scalar distribution,t= ',num2str(time)]);
filename=[bluePicPath,'/','scalar_distri','.eps'];
saveas(handle,filename);



handle=figure;
plot(yy,NQ_density*V/N,'-');
xlabel('x');
ylabel('num density');
title(['scalar distribution,t= ',num2str(time)]);
filename=[bluePicPath,'/','num_density','.eps'];
saveas(handle,filename);
